...
# Update the signature to accept a 'scaler' argument
def run_one_epoch(model, loader, criterion, optimizer, device, scaler=None, train=True):
    ...
    for clips, labels in pbar:
        # ... (keep your existing print/debug logic here if you want) ...
        ...
        # --- START OF CHANGE ---
        # 1. Use autocast for the forward pass
        # This automatically uses FP16 for compatible ops (convolutions, etc.)
        with torch.cuda.amp.autocast(enabled=(device.type == 'cuda')):
            logits = model(clips)
            loss = criterion(logits, labels)

        if train:
            # 2. Use the scaler for backward pass
            # This scales the loss to prevent underflow in FP16
            ...
            # Validation doesn't need scaling, just standard forward pass
            pass
        # --- END OF CHANGE ---
...
def main():
    ...
    # --- ADD THIS LINE ---
    # Create the GradScaler once at the beginning
    ...
    try:
        for epoch in range(1, args.epochs + 1):
            ...
            # --- UPDATE THESE CALLS ---
            # Pass the scaler to run_one_epoch
            train_loss, train_acc = run_one_epoch(
                model, train_loader, criterion, optimizer, device, scaler=scaler, train=True
            )
            # Validation doesn't update weights, but we pass scaler to keep signature same 
            # (or you can pass None, but passing scaler is safe)
            val_loss, val_acc = run_one_epoch(
                model, val_loader, criterion, optimizer, device, scaler=scaler, train=False
            )
            # --------------------------
